...
namespace Raydiance
{
	namespace Graphics
	{
		...
		private:
			...
			RHI_DX12_DescriptorSet(void);
			const Result Initialize(RHI_DX12_DescriptorPool* _descriptorPool, const RHI_DescriptorSetDescriptor* _descriptorSetDescriptor, D3D12_CPU_DESCRIPTOR_HANDLE _cpuHandle, D3D12_GPU_DESCRIPTOR_HANDLE _gpuHandle, uint32_t _handleSize);
