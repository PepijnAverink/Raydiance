...
namespace Raydiance
{
	namespace Graphics
	{
		RHI_DX12_FrameBuffer::RHI_DX12_FrameBuffer(RHI_DX12_RenderDevice* _renderDevice, const RHI_FrameBufferDescriptor* _frameBufferDescriptor)
			: RHI_FrameBuffer(_frameBufferDescriptor)
		{
			RHI_DX12_DescriptorHeapAllocation* allocation = _renderDevice->AllocateCPUDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV, _frameBufferDescriptor->Attachments.size());
			...
			for (uint32_t i = 0; i < _frameBufferDescriptor->Attachments.size(); i++)
			{
				...
				desc.Format = ResolveDX12ResourceFormat(_frameBufferDescriptor->Attachments[i].Texture->GetFormat());

				// Create View
				_renderDevice->GetD3DDevice()->CreateRenderTargetView(((RHI_DX12_Texture2D*)_frameBufferDescriptor->Attachments[i].Texture)->GetD3DTexture().Get(), &desc, ha);
				ha.ptr += m_BaseSize;
			}
		...
		RHI_DX12_FrameBuffer::~RHI_DX12_FrameBuffer()
		{
