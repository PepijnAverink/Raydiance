...
namespace Raydiance
{
	namespace Graphics
	{
		RHI_DX12_GraphicsPipeline::RHI_DX12_GraphicsPipeline(RHI_DX12_RenderDevice* _renderDevice, const RHI_GraphicsPipelineDescriptor* _graphicsPipelineDescriptor)
			: RHI_GraphicsPipeline(_graphicsPipelineDescriptor)
		{
			...
			psoDesc.DepthStencilState.DepthEnable = false;// TODO:: Implement depth testing! //  _graphicsPipelineDescriptor->DepthEnable;
			//if (_graphicsPipelineDescriptor->GetDepthStencilAttachment() != nullptr)  // TODO:: Implement depth texture
			...
			// create the pso
			HRESULT hr = ((RHI_DX12_RenderDevice*)_renderDevice)->GetD3DDevice()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_PipelineStateObj));
			if (hr != S_OK)
			{
				_com_error err(hr);
				LPCTSTR errMsg = err.ErrorMessage();
				Logger::Log("Failed to create GraphicsPipeline: " + std::string(err.ErrorMessage()), LogType::LOG_TYPE_ERROR);
				return;
			}
		}
