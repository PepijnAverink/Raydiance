...
namespace Raydiance
{
	namespace Graphics
	{
		...
		VKSwapchain::~VKSwapchain()
		{
			...
			vkDestroySwapchainKHR(((VKRenderDevice*)RHI_RenderDevice::Get().get())->GetDevice(), m_SwapChainObj, nullptr);
		}
		void VKSwapchain::Resize(CommandQueue* _commandQueue, const uint32_t _width, const uint32_t _height)
		{
			...
			// Create new
			VKRenderDevice* device = (VKRenderDevice*)RHI_RenderDevice::Get().get();
			CreateSwapchain(device, _commandQueue);
		}

		uint32_t VKSwapchain::AquireNewImage(CommandQueue* _commandQueue, RHI_Fence* _fence)
		{
			VkResult result = vkAcquireNextImageKHR(((VKRenderDevice*)RHI_RenderDevice::Get().get())->GetDevice(), m_SwapChainObj, UINT64_MAX, VK_NULL_HANDLE, ((VKFence*)_fence)->GetVKFence(), &m_CurrentBufferIndex);
