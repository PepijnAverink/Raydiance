...
namespace Raydiance
{
	namespace Graphics
	{
		RHI_VK_Buffer::RHI_VK_Buffer(RHI_VK_RenderDevice* _renderDevice, const RHI_BufferDescriptor* _bufferDescriptor)
			...
			, m_IndexFormat(ResolveVKIndexFormat(m_Layout.GetElements()[0].Type))
		{
			...
			if (vkCreateBuffer(_renderDevice->GetDevice(), &bufferInfo, nullptr, &m_BufferObj) != VK_SUCCESS)
				Logger::Log("VK_ERROR - Failed to create 'RHI_VK_Buffer' object.", LogType::LOG_TYPE_ERROR);
		...
		void RHI_VK_Buffer::SetData(void* _data, const uint32_t _size)
		{
			void* data;
		...
		uint32_t RHI_VK_Buffer::FindMemoryType(RHI_VK_RenderDevice* _renderDevice, uint32_t typeFilter, VkMemoryPropertyFlags properties) {
