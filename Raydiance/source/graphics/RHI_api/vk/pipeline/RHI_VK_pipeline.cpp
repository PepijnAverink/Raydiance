...
namespace Raydiance
{
	namespace Graphics
	{
		...
		RHI_VK_Pipeline::~RHI_VK_Pipeline()
		{
			vkDestroyPipeline(((RHI_VK_RenderDevice*)RHI_RenderDevice::Get())->GetVKDevice(), m_PipelineObj, nullptr);
		}


		const Result RHI_VK_Pipeline::Initialize(RHI_VK_RenderDevice* _RHI_RenderDevice, const RHI_ComputePipelineDescriptor* _computePipelineDescriptor)
		{
            Result result = RHI_Pipeline::Initialize(_computePipelineDescriptor);
            ...
            computeStageInfo.pName = _computePipelineDescriptor->Shader->GetEntryPointCStr();
            ...
            if (vkCreateComputePipelines(_RHI_RenderDevice->GetVKDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_PipelineObj) != VK_SUCCESS)
		...
		const Result RHI_VK_Pipeline::Initialize(RHI_VK_RenderDevice* _RHI_RenderDevice, const RHI_GraphicsPipelineDescriptor* _graphicsPipelineDescriptor)
		{
            Result result = RHI_Pipeline::Initialize(_graphicsPipelineDescriptor);
            if (CheckError(result) == true)
            {
                // Log error
                return result;
            ...
            if (vkCreateGraphicsPipelines(_RHI_RenderDevice->GetVKDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_PipelineObj) != VK_SUCCESS)
                Logger::Log("VK_ERROR - Failed to create 'VKGraphicsPipline' object.", LogLevel::LOG_LEVEL_ERROR);
