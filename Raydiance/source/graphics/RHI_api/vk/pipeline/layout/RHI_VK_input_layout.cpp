...
namespace Raydiance
{
	namespace Graphics
	{
		RHI_VK_InputLayout::RHI_VK_InputLayout(RHI_VK_RenderDevice* _renderDevice, const RHI_InputLayoutDescriptor* _inputLayoutDescriptor)
			: RHI_InputLayout(_inputLayoutDescriptor)
		{
			...
			for (int i = 0; i < _inputLayoutDescriptor->Layouts.size(); i++)
			{
				...
				else
				{
					...
					for (int j = 0; j < set.Elements.size(); j++)
					{
						...
						bindings[j].stageFlags = ResolveRHI_VK_ShaderTypeFlag(set.Elements[j].StageFlag);;
			...
			if (vkCreatePipelineLayout(_renderDevice->GetDevice(), &pipelineLayoutInfo, nullptr, &m_PipelineLayoutObj) != VK_SUCCESS)
				Logger::Log("VK_ERROR - Failed to create RHI_VK_InputLayout.", LogType::LOG_TYPE_ERROR);
		}
