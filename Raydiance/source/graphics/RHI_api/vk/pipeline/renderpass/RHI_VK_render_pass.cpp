...
namespace Raydiance
{
	namespace Graphics
	{
		RHI_VK_RenderPass::RHI_VK_RenderPass(RHI_VK_RenderDevice* _renderDevice, const RHI_RenderPassDescriptor* _renderPassDescriptor)
			: RHI_RenderPass(_renderPassDescriptor)
		{
			...
			for (uint32_t i = 0; i < _renderPassDescriptor->Attachments.size(); i++)
			{
				...
				attachment.loadOp = ResolveRHI_VK_RenderPassLoadOp(_renderPassDescriptor->Attachments[i].LoadOp);
				attachment.storeOp = ResolveRHI_VK_RenderPassStoreOp(_renderPassDescriptor->Attachments[i].StoreOp);
				...
				if (_renderPassDescriptor->Attachments[i].Type == RHI_RenderPassAttachmentType::RHI_RENDER_PASS_ATTACHMENT_TYPE_COLOR)
				{
				...
				else if (_renderPassDescriptor->Attachments[i].Type == RHI_RenderPassAttachmentType::RHI_RENDER_PASS_ATTACHMENT_TYPE_DEPTH_STENCIL)
				{
			...
			if (vkCreateRenderPass(_renderDevice->GetDevice(), &renderPassInfo, nullptr, &m_RenderPassObj) != VK_SUCCESS)
				Logger::Log("VK_ERROR - Failed to create 'RHI_VK_RenderPass' object.", LogType::LOG_TYPE_ERROR);
		}
